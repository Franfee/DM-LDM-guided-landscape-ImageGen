...
from nets.VAE512_512 import VAE
...
def Stage2_Train_UNet(local_rank, args):
    ...
    if LOAD_CHECK_POINT_UNET:
        unet1.load_state_dict(torch.load(os.path.join("result", "models", f"unet-{LOAD_UNET_IDX}.ckpt"), map_location='cpu'), strict=False)
        print("unet1 model load")
    else:
        pass
    ...
    # criterion_pred = torch.nn.L1Loss()
    ...
    for epoch in range(opt.s2start_epoch, opt.s2_epochs):
        ...
        if opt.checkpoint_interval != -1 and epoch % opt.checkpoint_interval == 0 and dist.get_rank() == 0:
            torch.save(unet1.module.state_dict(), os.path.join("result", "models", "unet-%d.ckpt" % epoch))
    
    if dist.get_rank() == 0:
        # end all epochs, train done
        torch.save(unet1.module.state_dict(), os.path.join("result", "models", "unet.pth"))
